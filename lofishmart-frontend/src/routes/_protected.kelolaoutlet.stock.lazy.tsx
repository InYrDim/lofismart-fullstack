...
function OutletStockPage() {
	...
	// Check if user has market/outlet assigned (for SPVR role)
	useEffect(() => {
	...
	const loadStocks = useCallback(async () => {
		...
		try {
			...
			// Filter to outlet-only stocks for this market
			const outletStocks = (res || []).filter(
				(s) => s.market?.id === activeMarketId,
			)
			setStocks(outletStocks);
		} catch (error) {
			console.error("Failed to fetch outlet stock list:", error);
		} finally {
			setLoading(false);
		}
	}, [activeMarketId]);
	...
	const outOfStock = filteredStocks.filter((s) => s.qty <= 0).length;
	...
	const handleAdjustStock = async () => {
		...
		try {
			...
			toast.error("Gagal memperbarui stok. Silakan periksa koneksi.");
		} finally {
	...
	const handleDeleteStock = async () => {
		...
		try {
			...
			console.error("Failed to delete outlet stock:", error);
	...
	return (
		<div className="flex flex-col h-full w-full overflow-hidden">
			<AppHeader
				title="Stok Outlet"
				description={auth?.marketName ? `Outlet: ${auth.marketName}` : "Pantau dan kelola stok barang di outlet Anda"}
			>
				<Button
					variant="outline"
					size="sm"
								...
								{loading ? (
										...
										return (
											...
											>
												...
												<TableCell className="py-3 text-right">
													<span
														className={`text-sm font-bold ${p.qty <= 0
															? "text-red-500"
															: p.qty < 10
																? "text-amber-500"
																: "text-emerald-600"
									...
									<TableRow>
										...
										>
											...
											<p>
												Tidak ada stok produk di outlet ini.
											</p>
										</TableCell>
									</TableRow>
								)}
			...
			>
				<DialogContent className="sm:max-w-[425px]">
					<DialogHeader>
						<DialogTitle>Sesuaikan Stok Outlet</DialogTitle>
						<DialogDescription>
							Memperbarui data stok{" "}
							<strong>{selectedStock?.product?.name}</strong> langsung
							ke database outlet.
						</DialogDescription>
					</DialogHeader>
